...
const TransactionForm = ({createTransaction})=>{
    ...
    const handleSubmit=(e)=>{
       ...
       const user = {item: item, amount: newAmount}
    ...
    return(
        <div className='transaction-form-container'>
            <form action="" className='transaction-form'>
                ...
                <div className='form-toggle'>
                    <button className={`toggle-income ${toggle===true ?'selected' :''}` } onClick={handleToggle}>
                        Income
                    </button>
                    <button className={`toggle-spending ${toggle===false ?'selected' :''}`} onClick={handleToggle}>
                        Expense
                    </button>
                </div>
                ...
                <div className='form-item'>
                    <label className='item-label'>Date (If obmitted will default to current date UTC)</label>
                        <input className='item-input-date' type="date" name="" id="" value={date} title='Default current date' onChange={handleDate}/>
                    <label className='item-label' htmlFor="">Item</label>
                        <input className='item-input' type="text" name='item' maxLength={30} value={item} onChange={handleUpdate}/>
                    <label htmlFor="" className='item-label' >Amount</label>
                        <input className='item-input' type="number" name='amount' placeholder='Enter Amount' min="0" value={amount} onChange={handleUpdate}/>
                </div>
                
                <button className='transaction-form-submit-btn' onClick={handleSubmit}>
                    Add transaction
                </button>      
